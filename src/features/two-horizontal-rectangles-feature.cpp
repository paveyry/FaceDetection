...
namespace violajones
{
  ...
  int TwoHorizontalRectanglesFeature::compute_value(std::shared_ptr<IntegralImage> image)
  {
  ...
  std::vector<std::shared_ptr<Feature>> TwoHorizontalRectanglesFeature::list_features()
  {
    std::vector<std::shared_ptr<Feature>> features;
    for (auto& rect : Window::list_features_positions(min_width, min_height))
      features.push_back(std::make_shared<TwoHorizontalRectanglesFeature>(rect));
    return features;
  }
}
